...
export class DbConnectorService {
  ...
  public async getVideojuegos(): Promise<videojuego[]> {
    return new Promise(async (ful) => {
      ...
      writeResult.records.forEach((record: { get: (arg0: string) => any }) => {
        const videojuego = record.get('juego').properties;
        this.setVideojuegos(videojuego).then((res) => {});
  ...
  public async setVideojuegos(videojuego: videojuego): Promise<any> {
    return new Promise(async (ful) => {
      const session = this.driver.session({ database: 'neo4j' });
      const writeQuery =
        ...
        videojuego.nombre +
        `'})
                          SET juego.recomendaciones = 3
                          RETURN juego.recomendaciones`;
      const writeResult = await session.executeWrite((tx: any) =>
        tx.run(writeQuery, {})
      );
      session.close();
